...
const rongCloudInit = (next)=>{
  const userLoginInfo = loginInfo();
  const userInfo = userLoginInfo.user;
  WARequest({
    url : `/systems/im/acheiveImUser/${userInfo.id}`,
    method : 'POST',
    data : {
      ...
      next && next(res.data.imToken);
    },
    fail : (err)=>{
      console.error('获取融云token失败！',err);
    }
  });
};

const BDReading = (text)=>{
  const userLoginInfo = loginInfo();
  const userInfo = userLoginInfo.user;
    ...
    data : {},
      ...
      if (window.env == 'wechat') {
        document.addEventListener("WeixinJSBridgeReady", ()=>{
          ...
          readingAudio.src = `https://tsn.baidu.com/text2audio?tex=${text}&tok=${data.access_token}&cuid=${userInfo.openid}&ctp=1&lan=zh`;
          readingAudio && readingAudio.play();
        }, false);
        //防止 WeixinJSBridgeReady 不起作用
        setTimeout(()=>{
          if(!readingAudio){
            readingAudio = new Audio();
            readingAudio.src = `https://tsn.baidu.com/text2audio?tex=${text}&tok=${data.access_token}&cuid=${userInfo.openid}&ctp=1&lan=zh`;
